...
GM.HelpMenuObjects = {
	...
	{ type = "image", display = "vgui/help/.png" },
...
function GM:OpenHelp( firstTime )
	...
	for k, v in pairs( self.HelpMenuObjects ) do
		...
		elseif v.type == "text" then
			...
			panel:SetSize( self.HelpScrollBar:GetWide(), markupheight + 8 ) --//How do I get the height of a markup object?
			panel.Paint = function()
				markupobj:Draw( 6, 4, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
			end
		elseif v.type == "image" then
			local ratio = self.HelpScrollBar:GetWide() / self.HelpMain:GetWide()
			panel:SetSize( self.HelpScrollBar:GetWide(), self.HelpMain:GetTall() * ratio )
