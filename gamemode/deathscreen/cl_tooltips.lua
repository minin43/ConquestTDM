...
Tooltips.MasterTable = {
    movement = { tip = "Enemies moving too quickly, jump-strafing, or bunny-hopping? Try out Frostbite to slow their movement down." },
    sniper = { tip = "Dying frequently to sniper rifles? Try damage-reducing perks, such and rebound or thornmail, or damage-reducing equipment like the Hyperweave vest." },
    explosive = { tip = "Dying to explosives too often? Try explosive-reducing equipment or the perk martyrdom." },
    ...
    if ply and ply:Alive() then
        ...
        if heldwep and heldwep:IsValid() then
            ...
            elseif !GAMEMODE.AmmoTipBlacklist or GAMEMODE.AmmoTipBlacklist != heldwep:GetClass() then
                ...
                if tab and (tab.slot == 1 or tab.slot == 2) then
                    if ply:GetAmmoCount( heldwep:GetPrimaryAmmoType() ) < 10 then
                        Tooltips.IncrementTip( "ammo" )
                    end
