...
hook.Add( "EntityTakeDamage", "Brainstorm1", function( ply, dmginfo )
	if CheckPerk( ply ) == "brainstorm" then
		...
		elseif dmginfo:GetDamage() < 26 then
			dmginfo:ScaleDamage( 1.1 )
		end
	end
end )

RegisterPerk( "Brainstorm", "brainstorm", 30, "Grants slightly increased movespeed, immunity to headshots, and resistance to high-damage weaponry, but become extra vulnerable to low-damage weaponry." )