...
if CheckPerk( ply ) == "murk" then
	...
	ply:SetRunSpeed ( 260 ) --default is 300
    ply:SetJumpPower( 135 ) --default is 170
    
    --[[if (ply:Team() == 1) then
		ply:SetModel( "models/csgobalkan1pm.mdl" )
	elseif (ply:Team() == 2) then
		ply:SetModel( "models/csgogign1pm.mdl" )
	end]]
...
hook.Add( "ScalePlayerDamage", "Murk", function( ply, hitgroup, dmginfo )
	if CheckPerk( ply ) == "murk" then
		if hitgroup == HITGROUP_CHEST then
			dmginfo:ScaleDamage( 0.5 ) --default is 1.0
		elseif hitgroup == HITGROUP_STOMACH then
			dmginfo:ScaleDamage( 0.5 ) --default is 1.0
        elseif hitgroup == HITGROUP_LEFTARM then
			dmginfo:ScaleDamage( 0.6 ) --default is 0.9
        elseif hitgroup == HITGROUP_RIGHTARM then
			dmginfo:ScaleDamage( 0.6 ) --default is 0.9
        elseif hitgroup == HITGROUP_LEFTLEG then
			dmginfo:ScaleDamage( 0.6 ) --default is 0.8
