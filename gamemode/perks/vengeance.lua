hook.Add( "EntityTakeDamage", "VengeanceDamageChecks", function( ply, dmginfo )
    ...
    if !att:IsPlayer() or att:Team() == vic:Team() then return end
			...
			if timer.Exists( "vengeance_" .. ply:SteamID() ) then
				...
				timer.Create( "delay_" .. ply:SteamID(), 2, 1, function()
					timer.Create( "vengeance_" .. ply:SteamID(), 0.3, 0, function()
						if ply:Alive() then
							local hp = ply:Health()
							if hp < hp + GAMEMODE.VengeanceHealthBack[ id( vic:SteamID() ) ] then
                                ply:SetHealth( hp + 1 )
                                GAMEMODE.VengeanceHealthBack[ id( vic:SteamID() ) ] = GAMEMODE.VengeanceHealthBack[ id( vic:SteamID() ) ] - 1
							end
						end
					end )
					timer.Destroy( "delay_" .. ply:SteamID() )
...
hook.Add( "PlayerDeath", "RemoveVengeanceRegen", function( ply )
	if timer.Exists( "vengeance_" .. ply:SteamID() ) then
        ...
        GAMEMODE.VengeanceHealthBack[ id( vic:SteamID() ) ] = 0
	end
	if timer.Exists( "delay_" .. ply:SteamID() ) then
		timer.Destroy( "delay_" .. ply:SteamID() )
	end
...
--RegisterPerk( "Vengeance", "vengeance", 0, "Deal bonus damage to your previous killer, heal back a small % of damage done by them.")