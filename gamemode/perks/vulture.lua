...
if CheckPerk( ply ) == "vulture" then
	ply.class = "Vulture"
    ply:SetWalkSpeed( 165 ) --default is 180
	ply:SetRunSpeed ( 310 ) --default is 300
    ply:SetJumpPower( 180 ) --default is 170
    
    --[[if (ply:Team() == 1) then
		ply:SetModel( "models/csgoleet1pm.mdl" )
	elseif (ply:Team() == 2) then
		ply:SetModel( "models/csgofbi4pm.mdl" )
	end]]
...
RegisterPerk( "Vulture", "vulture", 50, "Tired of enemies living with low life? Execute enemies below 10 HP! Enemies will also drop ammo packs when killed." )
