...
local notices = {
    ...
    --In the special case you suicide/kys, actually reset your killingspree data
    if att == vic then
        att.killsThisLife = 0
        return
    end
    ...
    if att.killcount > 1 then
		...
		elseif att.killcount == 5 then
            AddNotice(att, "ULTRA KILL", SCORECOUNTS.ULTRA_KILL, NOTICETYPES.SPECIAL, Color( 200, 0, 0 ))
            StopSounds(soundUltrakill)
        elseif att.killcount >= 6 then
            sc = SCORECOUNTS.UNREAL_KILL_MULTIPLIER * (att.killcount - 6) + SCORECOUNTS.MULTI_KILL
    ...
    --Start of killstreak announcer code; moved here to work with above StopSounds function
    vic.killsThisLife = 0
    if !att.killsThisLife then
        att.killsThisLife = 1
        return
    end
    
    att.killsThisLife = att.killsThisLife + 1
    
    if 	att.killsThisLife == 5 or att.killsThisLife == 6 or att.killsThisLife == 7 or 
		att.killsThisLife == 8 or att.killsThisLife == 9 or att.killsThisLife == 10 or 
		att.killsThisLife == 15 or att.killsThisLife == 20 or att.killsThisLife == 30 
        then
        StopSounds( sounds[att.killsThisLife] )
	    AddNotice( att, notices[att.killsThisLife], SCORECOUNTS["KILLSTREAK"..att.killsThisLife], NOTICETYPES.SPECIAL, Color( 200, 0, 0 ))
    end
    
    --First Blood
    if not firstBlood or firstBlood == nil then
        AddNotice(att, "FIRST BLOOD", SCORECOUNTS.FIRSTBLOOD, NOTICETYPES.EXTRA)
        StopSounds( soundFirstblood )
        firstBlood = 0
    end
